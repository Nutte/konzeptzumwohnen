<?php
/**
 * konzept functions and definitions
 *
...
if ( ! function_exists( 'konzept_setup' ) ) :
	/**
	...
	function konzept_setup() {
		 ...
		 * If you're building a theme based on konzept, use a find and replace
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'konzept' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'konzept_custom_background_args', array(
...
function konzept_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'konzept_content_width', 640 );
}
...
function konzept_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'konzept' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'konzept' ),
...
function konzept_scripts() {
	...
	wp_enqueue_script( 'konzept-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'konzept-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
